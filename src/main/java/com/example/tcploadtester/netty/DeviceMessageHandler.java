...
public final class DeviceMessageHandler extends SimpleChannelInboundHandler<String> {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        int delaySec = config.loginDelaySeconds() > 0
                ? ThreadLocalRandom.current().nextInt(config.loginDelaySeconds())
                : 0;
        log.debug("devId={} channel active gen={}, scheduling login 110 in {}s", session.devId(), connectionGeneration, delaySec);
